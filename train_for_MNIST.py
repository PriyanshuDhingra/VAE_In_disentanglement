...
def main():
    ...
    parser.add_argument('--dataset', choices=['mnist', 'mnist-U', 'mnist-N'], default='mnist-U',
                        help='MNIST datset to train/validate(default: mnist-U)')
    ...
    parser.add_argument('--encoder-kernel-number', type=int, default=128,
                        help='number of kernels in each layer of the encoder (default: 128)')
    parser.add_argument('--encoder-kernel-size', type=int, default=28,
                        help='size of kernels in the first layer of the encoder (default: 28)')
    parser.add_argument('--encoder-padding', type=int, default=8,
                        help='amount of the padding for the encoder (default: 8)')

    parser.add_argument('--in-channels', type=int, default=1, help='number of channels in the images (default:1)')
    parser.add_argument('--image-dim', type=int, default=50,
                        help='input image of the shape image_dim x image_dim (default:50)')
    ...
    parser.add_argument('--save-interval', default=2, type=int, help='save frequency in epochs (default: 20)')
    parser.add_argument('--num-epochs', type=int, default=10, help='number of training epochs (default: 500)')
    ...
    fourier_sigma = max(2.0 / (image_dim - 1), 2.0 / (
                image_dim - 1))  # setting sigma value of the fourier expansion to the pixel size in the image
    ...
    generator_model = models.SpatialGenerator(z_dim, generator_hidden_dim, num_layers=generator_num_layers,
                                              ...
                                              , resid=generator_resid, fourier_expansion=fourier_expansion,
                                              sigma=fourier_sigma)
    ...
    if t_inf == 'unimodal' and r_inf == 'unimodal':
        ...
        encoder_model = models.InferenceNetwork_UnimodalTranslation_UnimodalRotation(image_dim * image_dim, inf_dim,
                                                                                     encoder_kernel_number
                                                                                     , num_layers=encoder_num_layers,
                                                                                     activation=activation)

    elif t_inf == 'attention' and r_inf == 'unimodal':
        encoder_model = models.InferenceNetwork_AttentionTranslation_UnimodalRotation(image_dim, in_channels, z_dim,
        ...
        encoder_model = models.InferenceNetwork_AttentionTranslation_AttentionRotation(image_dim, in_channels, z_dim,
                                                                                       kernels_num=encoder_kernel_number
                                                                                       ,
