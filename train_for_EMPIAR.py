...
# For evaluating a batch of data for a model involving latent variable inference and transformations, including translation and rotation.
#expands and transforms the input data x and y as needed and prepares it for latent variable inference.
def eval_minibatch(x, y, ctf, generator_model, encoder_model, t_inf, r_inf, epoch, device
    ...
    # x (input coordinates) and y(ground truth data)
    # t_inf and r_inf represents inference types for translation and rotation
    # theta_prior, groupconv, padding, mask_radius: Parameters for priors, convolution groups, padding, and masking.
    b = y.size(0) # batch size
    n = int(y.size(-1)) # Image dimension size
    btw_pixels_space = (x[1, 0] - x[0, 0]).cpu().numpy() #Spacing between pixels
    x = x.expand(b, x.size(0), x.size(1)).to(device) # For compatibility with device
    y = y.to(device)

    if t_inf == 'unimodal' and r_inf == 'unimodal':
        y = y.view(b, -1)
        z_mu, z_logstd = encoder_model(y) #Obtain latent mean
        z_std = torch.exp(z_logstd) # Obtain latent standard deviation
        z_dim = z_mu.size(1)
        ...
        attn, attn_sampled, theta_vals, z_vals = encoder_model(y, device) # Attention weights are computed using the encoder_model. These weights determine how much each part of the input contributes to the output.
        ...
        # The same rotation matrix calculation is applied, but translation is handled using attention-based values.
...
#The KL divergence between the posterior distribution of the latent variables and their priors is computed using the attention-modulated latent variables.
        kl_div = val1 + val2
        kl_div = kl_div.mean()
        ...
        p_theta_given_t_r = Normal(offsets.unsqueeze(1).unsqueeze(2).to(device),
                                   torch.tensor([theta_prior_given_r] * attn.shape[1]).unsqueeze(1).unsqueeze(2).to(
                                       device))
    ...
    # Once the latent variables are sampled, they are passed into the generator_model to reconstruct the image: reconstruct
    ...
    if mask_radius > 0:
        ...
        dist = np.sqrt(
            (center[:, :, 0] - img_grid_batch[:, :, 0]) ** 2 + (center[:, :, 1] - img_grid_batch[:, :, 1]) ** 2)
...
# This function handles the training loop for one epoch of a Variational Autoencoder (VAE)
def train_epoch(iterator, x_coord, generator_model, encoder_model, optim, t_inf, r_inf
    ...
    # Theta prior is prior for rotational latent variables.
    # groupconv: Flag indicating whether to use group convolutions (for equivariant models).
    # mask_radius: Radius for masking parts of the image
    ...
    return elbo_accum, gen_loss_accum, kl_loss_accum # Return losses

# used for evaluation (validation/test)
...
# Loading images: Changed for loading differnt images instead of a single file to a np array
def load_images(path):
    ...
    if os.path.isdir(path):
        for file_name in os.listdir(path):
            ...
            if file_path.endswith(('mrc', 'mrcs')):
...
class Dataset:
    def __init__(self, y, ctf=None):
        if len(y) == 0:
            raise ValueError("Dataset is empty. No images were loaded.")
        self.y = y
        self.ctf = ctf
# Length of images
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        return self.y[idx], self.ctf[idx]
...
# Adding parameters
    ...
    parser.add_argument('--ctf-train',
                        help='path to CTF parameters for training images;or path to CTF parameters of whole set')
    ...
    parser.add_argument('-z', '--z-dim', type=int, default=1, help='latent variable dimension (default: 2)')
    parser.add_argument('--t-inf', default='attention', choices=['unimodal', 'attention'],
                        help='unimodal | attention (default:attention)')
    parser.add_argument('--r-inf', default='attention', choices=['unimodal', 'attention', 'attention+offsets'],
                        help='unimodal | attention | attention+offsets (default: attention+offsets)')
    parser.add_argument('--groupconv', type=int, default=4, choices=[0, 4, 8, 16], help='0 | 4 | 8 | 16 (default:8)')
    parser.add_argument('--encoder-num-layers', type=int, default=1,
                        help='number of hidden layers in the inference model when the translation and rotation inference are unimodal (default:2)')
    parser.add_argument('--encoder-kernel-number', type=int, default=1,
                        help='number of kernels in each layer of the encoder (default: 128)')
    parser.add_argument('--encoder-kernel-size', type=int, default=2,
                        help='size of kernels in the first layer of the encoder when using conv/groupconv layers (default: 64)')
    parser.add_argument('--encoder-padding', type=int, default=0,
                        help='amount of the padding for the encoder (default: 16)')
    parser.add_argument('--fourier-expansion', action='store_true',
                        help='using random fourier feature expansion in generator')
    parser.add_argument('--generator-hidden-dim', type=int, default=2,
                        help='dimension of hidden layers (default: 512)')
    parser.add_argument('--generator-num-layers', type=int, default=1, help='number of hidden layers (default: 2)')
    ...
    parser.add_argument('-l', '--learning-rate', type=float, default=1e-4, help='learning rate (default: 2e-4)')
    parser.add_argument('--minibatch-size', type=int, default=1, help='minibatch size (default: 100)')
    parser.add_argument('--train-portion', default=0.7, type=float,
                        help='portion of dataset used for training (default: 0.9)')
    parser.add_argument('--log-root', default='./training_logs',
                        help='path prefix to save models (default:./training_logs)')
    parser.add_argument('--save-interval', default=25, type=int, help='save frequency in epochs (default: 20)')
    parser.add_argument('--num-epochs', type=int, default=100, help='number of training epochs (default: 500)')
    ...
    parser.add_argument('--mask-radius', default=0, type=int,
                        help='radius of the circular mask for the reconstructed images (default:0)')
    ...
    if args.train_path:
        ...
        if args.ctf_train:
            ...
            if m % 2 == 0:
                ctf_m = m - 1
            ctf_params = load_ctf(args.ctf_train)
            ctf_filters = C.ctf_filter(ctf_params, ctf_n, ctf_m, scale=scale)
            if args.test_path and args.ctf_test:
                ctf_train = ctf_filters
                ctf_test_params = load_ctf(args.ctf_test)
                ctf_test = C.ctf_filter(ctf_test_params, ctf_n, ctf_m, scale=scale)
            else:
                train_size = int(ctf_filters.shape[0] * args.train_portion)
            ...
            print('CTF train shape:', ctf_train.shape)
            print('CTF test shape:', ctf_test.shape)

    else:
        print('please provide the train_path', file=sys.stderr)
        return
    if crop > 0:
    ...
    fourier_sigma = max(2.0 / (m - 1),
                        2.0 / (n - 1))  # setting sigma value of the fourier expansion to the pixel size in the image
    ...
    if t_inf == 'unimodal' and r_inf == 'unimodal':
        ...
        encoder_model = models.InferenceNetwork_UnimodalTranslation_UnimodalRotation(m * n, inf_dim,
                                                                                     encoder_kernel_number,
                                                                                     num_layers=encoder_num_layers,
                                                                                     activation=activation)

    elif t_inf == 'attention' and r_inf == 'unimodal':
        encoder_model = models.InferenceNetwork_AttentionTranslation_UnimodalRotation(m, in_channels, z_dim,
                                                                                      kernels_num=encoder_kernel_number,
                                                                                      activation=activation,
                                                                                      groupconv=group_conv)

    elif t_inf == 'attention' and (r_inf == 'attention' or r_inf == 'attention+offsets'):
        rot_refinement = (r_inf == 'attention+offsets')
        encoder_model = models.InferenceNetwork_AttentionTranslation_AttentionRotation(m, in_channels, z_dim,
                                                                                       kernels_num=encoder_kernel_number,
                                                                                       kernels_size=encoder_kernel_size,
                                                                                       padding=encoder_padding,
                                                                                       activation=activation,
                                                                                       groupconv=group_conv,
                                                                                       rot_refinement=rot_refinement,
    ...
    scheduler = ReduceLROnPlateau(optim, mode='max', factor=0.5, patience=9, threshold=1e-4, threshold_mode='abs',
    ...
    with open(path_prefix + 'train_log.txt', 'w', 1) as log_file:
        ...
        for epoch in range(num_epochs):
            ...
            elbo_accum, gen_loss_accum, kl_loss_accum = train_epoch(train_iterator, x_coord, generator_model,
                                                                    ...
                                                                    , num_epochs=num_epochs, N=N, device=device,
                                                                    params=params
